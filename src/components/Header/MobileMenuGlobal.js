...
const { icon, text, email, phone, socials, navItems,navItemsGlobal, navItemsTwo, lMobile } =
  headerData;

const MobileMenuGlobal = ({ onePage = false, number }) => {
  const { menuStatus, toggleMenu, menuStatus111, toggleMenu111 } =
    useRootContext();

  const newNavItems = onePage ? navItemsTwo : navItemsGlobal;
  ...
  return (
    <>
      <div className={`side-menu__block${menuStatus111 ? " active" : ""}`}>
        <div
...
export default MobileMenuGlobal;
